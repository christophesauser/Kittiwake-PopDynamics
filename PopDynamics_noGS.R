...
for (i in seq_len(nrow(meta))) {
  ...
  post[[col]] <- list(
    ...
    lat       = rep(meta$latitude[i], length(idx)),
    phi_draws = phi_draws,
    fe_draws  = fe_draws,
    sp_draws  = sp_draws,
    sf_draws  = sf_draws,
...
post <- post[!names(post) %in% c("Grumant", "SorGjeslingan")]
colonies_ok <- names(post)
...
eiv_phi_lat   <- eiv_regression(post, "lat", "phi_draws")
eiv_fe_lat    <- eiv_regression(post, "lat", "fe_draws")
eiv_sphi_lat  <- eiv_regression(post, "lat", "s_phi")
eiv_sfe_lat   <- eiv_regression(post, "lat", "s_fe")
eiv_esurv_lat <- eiv_regression(post, "lat", "e_surv")
eiv_erecr_lat <- eiv_regression(post, "lat", "e_recr")


write.csv(S, "colony_summary_v5_noGS.csv", row.names = FALSE)